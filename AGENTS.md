...
사용자 메시지 맨 앞 또는 본문에 `@ask` 또는 `/ask`가 포함되었거나, 현재 세션이 ask 모드로 고정된 경우 발동되는 순수 질의응답/읽기 전용 모드입니다.
...
사용자 메시지에 `@agent` 또는 `/agent`가 포함되었거나, 아무 데코레이터가 없는 기본 상태입니다.
...
- 사용자가 설정한 목표가 100% 달성될 때까지 디버깅, 테스트, 자가 수정을 거듭하며 작업을 중단하지 않고 끝까지 수행합니다.
- 복잡하거나 시간이 걸리는 작업에 적합합니다.

---

## 5. 대화 세션 모드 고정 (Sticky Session Mode)
매번 프롬프트마다 멘션을 치지 않아도 되도록 다음을 지원합니다:
