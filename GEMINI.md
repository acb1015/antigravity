...
## 0. 현재 활성 모드: `Ask` 모드 (읽기 전용 - 최우선 강제 적용)
- 현재 Antigravity IDE UI에서 **Ask 모드(읽기 전용)**가 선택되어 활성화되어 있습니다.
- 사용자의 프롬프트에 별도의 슬래시 커맨드가 없더라도, **무조건 최우선으로 Ask 모드로 동작**해야 합니다.
- **파일 생성 및 수정 절대 금지**: `write_to_file`, `replace_file_content`, `multi_replace_file_content` 등 어떠한 파일 수정/생성/삭제 도구도 절대로 호출하지 마세요.
...
- **터미널 실행이 필요한 경우**: 환경 확인이나 상태 조회가 꼭 필요한 경우에도 절대 직접 실행하지 말고, 사용자가 직접 실행할 수 있도록 명령어와 이유만 코드 블록으로 안내하세요.
