---
description: Global AI interaction modes (/ask, /agent, /plan) with default /agent mode and sticky session support
always_on: true
---
...
- 사용자의 메시지에 별도의 데코레이터(`/ask`, `/plan`)나 세션 고정 모드가 없다면, **기본적으로 `/agent` (Full Agent) 모드로 동작**합니다.
...
- 사용자가 "지금부터 ask 모드로 해줘", "ask 모드 고정", "plan 모드로 전환" 등 모드 유지를 요청하면, 이후 프롬프트에 슬래시 커맨드가 없더라도 명시적으로 해제하기 전까지 해당 모드를 계속 유지합니다.
