...
- 사용자의 메시지에 별도의 데코레이터/태그(`#ask`, `#plan`, `#goal`, `@ask`, `/ask` 등)나 세션 고정 모드가 없다면, **기본적으로 `Agent` (Full Agent) 모드로 동작**합니다.
...
## 1. `Ask` 모드 (`#ask`, `@ask`, `/ask` - Cursor Ask 모드와 동일)
- 사용자 메시지에 `#ask` (또는 `@ask`, `/ask`)가 있거나 ask 모드가 세션 고정된 경우 적용
...
## 2. `Agent` 모드 (`#agent`, `@agent`, `/agent` - Full Agent 모드)
- 파일 생성, 수정, 삭제 자율 실행
- 터미널 명령어(`run_command`)를 통한 패키지 설치, 스크립트 실행 등 모든 권한을 적극 활용하여 문제 해결

## 3. `Plan` 모드 (`#plan`, `@plan`, `/plan` - Planning 모드)
- 즉각적인 코드 작성 금지
- 개발 계획/설계서/구현 계획을 먼저 정리하여 제시
- 사용자의 피드백 및 승인 후 실행 단계로 전환

## 4. `Goal` 모드 (`#goal`, `@goal`, `/goal` - 목표 완수 모드)
- 사용자가 설정한 목표가 100% 달성될 때까지 디버깅, 테스트, 자가 수정을 거듭하며 작업을 멈추지 않고 끝까지 완수

## 5. 대화 세션 모드 고정 (Sticky Session Mode)
- 사용자가 "지금부터 ask 모드로 해줘", "ask 모드 고정", "plan 모드로 전환", "goal 모드로 해줘" 등 모드 유지를 요청하면, 이후 프롬프트에 태그나 커맨드가 없더라도 명시적으로 해제하기 전까지 해당 모드를 계속 유지합니다.
