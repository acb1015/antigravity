...
채팅창에서 **`@`**를 치면 자동완성 힌트 목록에 **`ask`**, **`agent`**, **`plan`**, **`goal`**이 표시됩니다. (기존 슬래시 커맨드 `/ask`, `/agent`, `/plan`, `/goal`도 동일하게 작동합니다.)

| 모드 | 사용 방법 | 설명 | 권한 및 동작 |
| :--- | :--- | :--- | :--- |
| **Full Agent (기본)** | 별도 멘션 없음 또는 `@agent` (`/agent`) | AI가 스스로 판단하여 문제를 끝까지 해결하는 완전 자율 모드 | 파일 생성/수정/삭제 허용, 터미널 명령어 실행 허용 |
...
| **Goal (목표 완수)** | `@goal <목표>` (또는 `/goal <목표>`) | 목표가 100% 달성될 때까지 멈추지 않고 끈질기게 완수하는 모드 | 장기 실행, 에러 자가 치유(Self-Healing) 및 최종 검증까지 자율 완수 |
